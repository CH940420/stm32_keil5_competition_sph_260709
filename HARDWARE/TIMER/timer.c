...
//  TIM_ICInit(TIM5, &TIM5_ICInitStructure);
	...
	TIM_ITConfig(TIM5,TIM_IT_Update,ENABLE);//允许更新中断 ,允许CC1IE捕获中断	|TIM_IT_CC1
...
//		}			     	    					   
// 	}
	TIM_ClearITPendingBit(TIM5, TIM_IT_Update); //清除中断标志位TIM_IT_CC1|
} 
//TIM9 CH2 PWM输出设置 
//PWM输出初始化
//arr：自动重装值
...
{		 					 
	...
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource6,GPIO_AF_TIM9); //GPIOA2复用为定时器9
//	GPIO_PinAFConfig(GPIOA,GPIO_PinSource3,GPIO_AF_TIM9); //GPIOA3复用为定时器9
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;           //GPIOA2,PA3
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;        //复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;	//速度100MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;      //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;        //上拉
	GPIO_Init(GPIOE,&GPIO_InitStructure);              //初始化PA2,PA3
