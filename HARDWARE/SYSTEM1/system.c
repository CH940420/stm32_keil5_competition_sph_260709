...
//Default speed of remote control ship, unit: mm/s
//遥控小船的默认速度，单位：mm/s
...
//语音、ROS、航模手控。这3个标志位默认都为0，代表串口3控制模式
u8 YY_ON_Flag=0, ROS_ON_Flag=0, Remote_ON_Flag=0, Usart1_ON_Flag, Usart5_ON_Flag; 

u8 Remote_SPEED_ON_Flag=0, Remote_SPEED_SET_Flag=0; 
...
//	mpu_dmp_init();
	//PA3
//	TIM9_PWM_Init(20000-1,16.8-1);//84M/84=1Mhz的计数频率,重装载值20000，所以PWM频率为 1M/20000=50hz.  
  TIM9_PWM_Init(20000-1,168-1);//84M/84=1Mhz的计数频率,重装载值20000，所以PWM频率为 1M/20000=50hz.	
	//PB15
	//TIM12_CH2_PWM_Init(20000-1,16.8-1);	//84M/84=1Mhz的计数频率,重装载值20000，所以PWM频率为 1M/20000=50hz.     
  
	TIM12_CH2_PWM_Init(20000-1,84-1);	//84M/84=1Mhz的计数频率,重装载值20000，所以PWM频率为 1M/20000=50hz.
	TIM2_Int_Init(1000-1,168-1);	//1ms  //周期1ms调度不定长任务
...
//	 ch_wal_old[i]=1000;
	...
	TIM5_CH1_Cap_Init(10000-1,168-1);//  T=168*1000/168M  //自动模式需要提前手动模式抽空空气。10ms延时喷水投料
	
	//读flash数据，存有自动模式为1，还有地位
